...
/// Creates an minutely, rolling file appender.
///
...
mod test {
    ...
    fn test_next_date_minutely() {
        ...
        let next = r.next_date(&mock_now);
        assert_eq!(mock_now.with_hour(0).unwrap().with_minute(21).unwrap().with_second(0).unwrap(), next);
        ...
        let next = r.next_date(&mock_now);
        assert_eq!(mock_now.with_day(2).unwrap().with_hour(0).unwrap().with_minute(0).unwrap(), next);
